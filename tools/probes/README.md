...
```bat
rem 把 QT_DIR / MINGW_DIR 换成你自己的 Qt 5.8 mingw53_32 与 MinGW 5.3 安装目录
...
g++ -std=gnu++11 -o p6.exe 06_about_to_quit.cpp ^
    ...
    -L%QT_DIR%/lib -lQt5Core -lQt5Gui -lQt5Widgets && p6.exe
```
...
| `06_about_to_quit.cpp` | `QApplication::quit()` 会不会触发 `aboutToQuit`；`QSettings` 写得进去吗 | **会触发**（Qt 5.8 实测），设置可以挂在该信号上落盘；同时测出**受限环境下 HKCU 写入可能被静默拒绝**——`QSettings` 不报错，但也不生效 |
...
实测输出（一块 5400 RPM 机械盘，PhysicalDrive0）：
